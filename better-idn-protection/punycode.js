// Minimal RFC3492 Punycode implementation exported as global `punycode`.
// Extracted from the previous app.js implementation for reuse across scripts.
...
var punycode = new function Punycode() {
  ...
  this.decode = function(input, preserveCase) {
    ...
    if (preserveCase) {
      for (i = 0; i < output.length; i++) {
        if (case_flags[i]) output[i] = (String.fromCharCode(output[i]).toUpperCase()).charCodeAt(0);
      }
    }
  ...
  this.encode = function(input, preserveCase) {
    ...
    var input_length = input.length;
    if (preserveCase) {
      for (j = 0; j < input_length; j++) case_flags[j] = input[j] != case_flags[j];
    }
    var output = [];
    n = initial_n;
    delta = 0;
    ...
    for (j = 0; j < input_length; ++j) {
      if (input[j] < 0x80) output.push(String.fromCharCode(case_flags ? encode_basic(input[j], case_flags[j]) : input[j]));
    }
    h = b = output.length;
